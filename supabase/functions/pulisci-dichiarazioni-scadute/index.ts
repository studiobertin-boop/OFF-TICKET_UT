...
/**
 * Edge Function: pulisci-dichiarazioni-scadute
 *
 * Gemella di `pulisci-fascicoli-scaduti`, stessa struttura e stesso cron giornaliero, ma per
 * `dichiarazioni_documenti`/`dichiarazioni_scadenze`/`dichiarazioni_movimenti` e il bucket
 * `dichiarazioni`. Nessun concetto di "orfani per apparecchiatura eliminata": qui la
 * granularità è la pratica intera, non l'apparecchiatura — non esiste un codice a cui i
 * documenti possano restare agganciati dopo che il codice sparisce.
 *
 * La regola di scadenza è la stessa identica di `src/services/fascicolo/scadenza.ts` — non
 * duplicata: due copie divergerebbero.
 *
 * Riusa gli stessi secret del Vault della funzione gemella (`service_role_key`,
 * `fascicolo_cron_secret`): sono un'autorizzazione interna al progetto, non specifica al
 * fascicolo, e i secret delle Edge Function sono condivisi a livello di progetto — non serve
 * configurarne di nuovi per questa funzione.
 */

const BUCKET = 'dichiarazioni'
...
serve(async (req) => {
  ...
  try {
    ...
    }>(supabase, 'dichiarazioni_movimenti', '*', 'request_id')
    ...
    for (const m of movimenti) {
      try {
        ...
        if (stato.scaduta) {
          ...
          const { error: erroreScadenza } = await supabase.from('dichiarazioni_scadenze').upsert(
...
/**
 * Avvisa gli `userdm329` e gli admin, una volta sola per finestra di preavviso. Non il tecnico
 * assegnato: sono gli `userdm329` a lavorare la pratica nel tempo, vedi `fascicolo`.
 */
async function preavvisaSeServe(
  ...
  const { data: gia, error: erroreLettura } = await supabase
    ...
    .eq('type', 'dichiarazioni_in_scadenza')
    .gte('created_at', da)
    .limit(1)
  if (erroreLettura) throw erroreLettura
  ...
  const { error: erroreInvio } = await supabase.from('notifications').insert(
    [...destinatari].map((user_id) => ({
      ...
      type: 'dichiarazioni_in_scadenza',
      event_type: 'dichiarazioni_in_scadenza',
      read: false,
      message: `I documenti delle dichiarazioni di «${pratica?.title ?? 'una pratica'}» verranno cancellati il ${giorno}. Scaricali se ti servono.`,
    }))
  )
