-- Add timer alert system for DM329 requests
-- Monitors time spent in specific states and flags requests exceeding thresholds

-- Add has_timer_alert field to requests table
ALTER TABLE requests
ADD COLUMN has_timer_alert BOOLEAN DEFAULT false;

-- Create index for filtering timer alerts
CREATE INDEX idx_requests_timer_alert ON requests(has_timer_alert) WHERE has_timer_alert = true;
