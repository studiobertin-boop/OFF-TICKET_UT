-- Manual application script for ARCHIVIATA NON FINITA status
-- This script applies the new migrations for the archived status

-- Step 1: Add new status to dm329_status ENUM
DO $$
BEGIN
  -- Add the new value to the dm329_status ENUM if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM pg_enum e
    JOIN pg_type t ON e.enumtypid = t.oid
    WHERE t.typname = 'dm329_status'
    AND e.enumlabel = 'ARCHIVIATA NON FINITA'
  ) THEN
    ALTER TYPE dm329_status ADD VALUE 'ARCHIVIATA NON FINITA';
    RAISE NOTICE 'Added ARCHIVIATA NON FINITA to dm329_status enum';
  ELSE
    RAISE NOTICE 'ARCHIVIATA NON FINITA already exists in dm329_status enum';
  END IF;
END
$$;

-- Step 2: Update validate_status_transition function
...
COMMENT ON FUNCTION validate_status_transition IS 'Validates status transitions based on user role, request type workflow, and blocked state. Admin: any status. Userdm329: any DM329 status (including ARCHIVIATA NON FINITA). Tecnico: only general requests with workflow. Utente: cannot change status.';
...
BEGIN
  ...
  RAISE NOTICE 'ARCHIVIATA NON FINITA enum value exists: %', v_enum_exists;
  RAISE NOTICE 'validate_status_transition function exists: %', v_function_exists;

  IF v_enum_exists AND v_function_exists THEN
    RAISE NOTICE 'SUCCESS: All changes applied successfully!';
  ELSE
    RAISE WARNING 'INCOMPLETE: Some changes may not have been applied';
  END IF;
END
$$;
