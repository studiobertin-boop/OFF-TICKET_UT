-- =====================================================
-- MANUAL MIGRATION: Deletion Archive System
-- =====================================================
-- ISTRUZIONI:
...
-- PART 1: Create deletion_archives table
-- =====================================================
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'deletion_archives') THEN
    ...
    RAISE NOTICE 'Table deletion_archives created successfully';
  ELSE
    RAISE NOTICE 'Table deletion_archives already exists, skipping...';
  END IF;
END $$;
...
-- COMPLETED!
-- =====================================================
-- Se vedi questo messaggio senza errori, la migration è stata applicata con successo.
-- Verifica che:
-- 1. La tabella 'deletion_archives' esiste
-- 2. Il bucket storage 'deletion-archives' esiste
-- 3. Le policies RLS sono attive
