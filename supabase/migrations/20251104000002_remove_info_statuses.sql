-- Migration: Remove INFO_NECESSARIE and INFO_TRASMESSE statuses
-- Date: 2025-11-04
-- Description: Removes unused status transitions and updates validation function

-- =====================================================
-- PART 1: Update validate_status_transition function
-- =====================================================
...
COMMENT ON FUNCTION validate_status_transition IS 'Validates status transitions. Removed INFO_NECESSARIE and INFO_TRASMESSE states from workflow.';

-- =====================================================
-- PART 2: (Optional) Update existing requests with removed statuses
-- =====================================================
-- Uncomment the following lines if you want to migrate existing data:

-- Update requests with INFO_NECESSARIE to IN_LAVORAZIONE
-- UPDATE requests
-- SET status = 'IN_LAVORAZIONE', updated_at = NOW()
-- WHERE status = 'INFO_NECESSARIE';

-- Update requests with INFO_TRASMESSE to IN_LAVORAZIONE
-- UPDATE requests
-- SET status = 'IN_LAVORAZIONE', updated_at = NOW()
...
-- COMPLETED
-- =====================================================
-- The workflow has been updated to remove INFO_NECESSARIE and INFO_TRASMESSE.
-- New status flow: APERTA → ASSEGNATA → IN_LAVORAZIONE → COMPLETATA
--                  With optional: SOSPESA, ABORTITA
