...
/**
 * Composizione del PDF finale: tutto A4, tutto entro il limite di peso.
 *
 * L'ordine dei documenti arriva già deciso da `ordinaFascicolo`: qui si decide solo *come*
 * ogni pagina si posa sul foglio e *quanto* si può ridurre prima di consegnare.
 */

/** A4 verticale in punti tipografici, l'unità di misura del PDF. */
export const A4 = { larghezza: 595.28, altezza: 841.89 } as const
...
/** Posa di un contenuto dentro il foglio A4, nel sistema di riferimento di pdf-lib. */
...
/**
 * Colloca un contenuto di `w × h` dentro un A4 verticale, sfruttando il foglio il più possibile.
 *
 * Ciò che è più largo che alto viene ruotato di 90°: una targhetta fotografata in orizzontale,
 * ridotta senza ruotare, occuperebbe una striscia alta un terzo di pagina e si leggerebbe con
 * la lente. Ruotata riempie il foglio, e il foglio si gira.
 *
 ...
 */
export const inquadraInA4 = (w: number, h: number): Inquadratura => {
  const { larghezza: W, altezza: H } = A4
  const ruotato = w > h
...
/** Aggiunge un foglio A4 al fascicolo e restituisce foglio e inquadratura del contenuto. */
const nuovoFoglio = (out: PDFDocument, w: number, h: number): [PDFPage, Inquadratura] => {
  const pagina = out.addPage([A4.larghezza, A4.altezza])
  return [pagina, inquadraInA4(w, h)]
}
 ...
 */
const aggiungiPaginePdf = async (out: PDFDocument, file: File) => {
  ...
  for (const indice of sorgente.getPageIndices()) {
    ...
    const [foglio, q] = nuovoFoglio(out, incorporata.width, incorporata.height)
    posa(foglio, q, (o) => foglio.drawPage(incorporata, o))
  }
}
...
const aggiungiImmagine = async (
  ...
  const [foglio, q] = nuovoFoglio(out, immagine.larghezza, immagine.altezza)
  posa(foglio, q, (o) => foglio.drawImage(incorporata, o))
}
...
const componiUnaVolta = async (
  ...
  for (const s of sorgenti) {
    ...
    try {
      if (eUnPdf(s.file)) {
        if (!riduzione) {
          await aggiungiPaginePdf(out, s.file)
        } else {
          for (const pagina of await rasterizzaPdf(s.file, riduzione)) {
            await aggiungiImmagine(out, pagina)
          }
        }
        continue
      }
      ...
      if (intatta) {
        ...
        await aggiungiImmagine(out, await ricomprimiImmagine(s.file, riduzione ?? undefined))
      }
    } catch (errore) {
      scartati.push({
...
/**
 * Rilega i documenti in un unico PDF A4 che sta sotto il limite di peso.
 *
 * Si prova prima senza toccare nulla: quasi sempre basta, e il fascicolo esce con i certificati
 * ancora vettoriali. Solo se sfora si riduce, un grado alla volta, fermandosi al primo che
 * rientra — così il fascicolo perde il minimo indispensabile e non il massimo consentito.
 */
export const componiFascicolo = async (
  sorgenti: SorgenteFascicolo[],
  { limiteByte = LIMITE_BYTE, onProgresso }: OpzioniComposizione = {}
  ...
  for (let i = 0; i < gradi.length; i++) {
    ...
    ultimo = await componiUnaVolta(sorgenti, grado)
    ridotti = i === 0
      ? []
      : sorgenti
