/**
 * Riduzione di immagini e pagine a JPEG, per far rientrare un documento composto nel limite di peso.
 *
 * È l'unica parte della composizione che ha bisogno del browser: disegna su `canvas`. Il resto —
...
const contesto2d = (canvas: HTMLCanvasElement) => {
  ...
  if (!ctx) throw new Error('Il browser non ha concesso un contesto 2D per comporre il documento')
  return ctx
}
 ...
 *
 * Si usa solo quando il documento non rientra nel limite di peso nemmeno dopo aver ridotto le
...
export const rasterizzaPdf = async (
  ...
  // ma non nell'ambiente dei test. Importandolo qui, la composizione resta verificabile senza un
  // browser — e questa è l'unica funzione che pdfjs le serve davvero.
